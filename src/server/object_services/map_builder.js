...
class MapBuilder {
    ...
    async run_build(chunk_ids) {
        ...
        this.chunks = [];
        this.db_chunks = undefined;
        this.chunks.map(chunk => new ChunkDB(chunk));
    ...
    async reload_chunks(chunk_ids) {
        ...
        this.db_chunks = this.chunks.map(chunk => new ChunkDB(chunk));
    ...
    async prepare_and_fix_chunks() {
        ...
        _.each(deleted_chunks, chunk => {
            _.each(chunk.frags, frag => {
                const live_blocks = frag.blocks.filter(block => !block.deleted);
        ...
        await P.map(this.chunks, async chunk => {
            ...
            try {
                ...
                if (!chunk.tier) { // tier was deleted?
                    ...
                    chunk.tier = mapper.select_tier_for_write(bucket.tiering, tiering_status)._id;
                }
            } catch (err) {
    ...
    async build_chunks() {
        ...
        try {
            ...
            if (mc.had_errors) {
                this.had_errors = true;
                const mapped_chunks = mc.get_mapping();
                for (let i = 0; i < this.chunks.length; ++i) {
                    if (mapped_chunks[i].had_errors) {
                        dbg.error('MapBuilder.build_chunks: mark errors on chunk', mapped_chunks[i]._id);
                        chunks[i].had_errors = true;
                    }
                }
            }
    ...
    update_db() {
        const success_chunk_ids = mongo_utils.uniq_ids(
            _.filter(this.db_chunks, chunk => !chunk.had_errors && !chunk.bucket.deleted), '_id');
        ...
        return Promise.all([
            ...
            objs_to_be_deleted.map(async obj => {
                try {
